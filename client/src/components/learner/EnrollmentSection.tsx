    ...
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 border-b pb-2 mb-4 border-emerald-200">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest text-emerald-700">Current Enrollment</h2>
        </div>

        {currentEnrollment ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
             <div className="grid grid-cols-[140px_1fr] gap-4 items-center py-2 border-b border-muted/30">
               <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">School Year</span>
               <span className="text-sm font-bold">{learner.schoolYear?.yearLabel || "N/A"}</span>
             </div>
             <div className="grid grid-cols-[140px_1fr] gap-4 items-center py-2 border-b border-muted/30">
               <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Grade Level</span>
               <span className="text-sm font-bold">{learner.gradeLevel?.name || "N/A"}</span>
             </div>
             <div className="grid grid-cols-[140px_1fr] gap-4 items-center py-2 border-b border-muted/30">
               <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Section</span>
               <span className="text-sm font-bold">{currentEnrollment.section?.name || "N/A"}</span>
             </div>
             <div className="grid grid-cols-[140px_1fr] gap-4 items-center py-2 border-b border-muted/30">
               <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Class Adviser</span>
               <span className="text-sm font-bold">
                 {currentEnrollment.section?.advisingTeacher ? 
                   `${currentEnrollment.section.advisingTeacher.lastName}, ${currentEnrollment.section.advisingTeacher.firstName}` : "N/A"}
               </span>
             </div>
             <div className="grid grid-cols-[140px_1fr] gap-4 items-center py-2 border-b border-muted/30">
          ...
          <p className="text-sm text-muted-foreground italic">No active enrollment record found.</p>
        )}
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 border-b pb-2 mb-4 border-slate-200">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest text-slate-700">Enrollment History</h2>
        </div>
            ...
            <tbody className="divide-y divide-muted/30">
              {history.map((h: any, i: number) => (
                <tr key={i} className="hover:bg-muted/10 transition-colors">
                  <td className="px-4 py-3 font-semibold text-xs">{h.schoolYear}</td>
